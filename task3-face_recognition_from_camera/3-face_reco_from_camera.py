...
class Face_Recognizer:
    ...
    def draw_note(self, img_rd):
        ...
        # cv2.putText(img_rd, "Frame:  " + str(self.frame_cnt), (20, 100), self.font, 0.8, (187, 255, 255), 1,cv2.LINE_AA)
        # cv2.putText(img_rd, "FPS:    " + str(self.fps_show.__round__(2)), (20, 130), self.font, 0.8, (0, 255, 0), 1,cv2.LINE_AA)
        cv2.putText(img_rd, "Faces:  " + str(self.current_frame_face_cnt), (20, 160), self.font, 0.8, (187, 255, 255), 1,
                    cv2.LINE_AA)
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                if kk == ord('q'):
                    ...
                    #  检测到人脸 / Face detected in current frame
                    if len(faces) != 0:
                        #  获取当前捕获到的图像的所有人脸的特征 / Compute the face descriptors for faces in current frame
                        for i in range(len(faces)):
                            shape = predictor(img_rd, faces[i])
                            self.current_frame_face_feature_list.append(face_reco_model.compute_face_descriptor(img_rd, shape))
                        #  遍历捕获到的图像中所有的人脸 / Traversal all the faces in the database
                        for k in range(len(faces)):
                            ...
                            #  寻找出最小的欧式距离匹配 / Find the one with minimum e-distance
